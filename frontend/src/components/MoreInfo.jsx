...
const MoreInfo = () => {
  ...
  <div className='grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] justify-center gap-8 pt-5 gap-y-6'>
        { service_list.slice(0,4).map((item, index) => (
            <div onClick={()=> {navigate(`/appointment/${item._id}`); scrollTo(0,0)}} className='shadow-navbar rounded-2xl cursor-pointer hover:scale-105 hover:z-50 transition-all duration-400' key={index}>
