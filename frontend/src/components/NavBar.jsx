...
const NavBar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm p-5 mb-5 border-b border-b-blue-100'>
        <img className='w-15 cursor-pointer' src={assets.logo} alt="" />
        <ul className='md:flex items-start gap-5 text-[1.25rem] text-primary-light sm:hidden'>
             <NavLink to='/'>
                <li className='py-1'>Home</li>
                <hr className='border-none outline-none h-[1.5px] bg-primary-light w-10 m-auto hidden'/>
             </NavLink>
             <NavLink to='/services'>
        ...
        <div className="flex items-center gap-4">
            {
               ...
               ?<div className='flex items-center gap-2 cursor-pointer group relative'>
                  <img className='w-10 rounded-full border-2 border-whitesmoke' src={assets.profile_pic} alt="" />
               ...
               :<button onClick={() => navigate('/login') } className='bg-primary-light text-white px-3 py-2 rounded-full font-light hidden md:block'>Create account</button>
            }
        </div>
    </div>
  )
